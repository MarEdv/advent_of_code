...
(defn checkSum-part1 []
  (let [diffFunc (fn [a] (- (apply max a) (apply min a)))
        list (map diffFunc spreadSheet)]
    (reduce + list)
    )
  )

(defn divisible [a]
  (= (mod (first a) (second a)) 0)
  )

(defn different [a]
  (not= (first a) (second a))
  )

(defn diffFunc2 [lst]
  (->>
    (for [x lst, y lst] (list x y))
    (filter divisible)
    (filter different)
...
(defn checkSum-part2 []
  (let [lst (map diffFunc2 spreadSheet)]
    (reduce + lst)
    )
  )
